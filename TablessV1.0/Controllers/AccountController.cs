    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register([Bind(Exclude = "UserPhoto")]RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Email, Fname = model.Fname, Lname = model.Lname, Email = model.Email, profile_photo = model.profile_photo, facebook_connect = model.facebook_connect, twitter_connect = model.twitter_connect, google_connect = model.google_connect ,GoogleUserId=model.GoogleUserId};
                user.profile_photo = ImageToByteArray(System.Drawing.Image.FromFile(Server.MapPath("~/Images/5.png")));
        ...
        //[HttpPost]
        //public ActionResult Account([Bind(Exclude = "UserPhoto")]RegisterViewModel model)
        //{
        //    if (ModelState.IsValid)
        //    {
        ...
        //        var user = new ApplicationUser { UserName = model.Email, Email = model.Email, profile_photo = model.profile_photo };
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Account(HttpPostedFileBase Profile)
        {
            try
            {
                var db = HttpContext.GetOwinContext().Get<ApplicationDbContext>();
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    if (user != null)
                    {
                        ...
                        await SignInAsync(user, isPresistent: false);
                    }
        ...
        private async Task SignInAsync(ApplicationUser user, bool isPresistent)
        {
